...
def initialize_nest(sim_params):
  ...
  ### global nest rng ###
  nest.SetKernelStatus({'rng_seed' : sim_params['msd']+N_vp})
  ### per process rng #####
#   nest.SetKernelStatus({'rng_seeds' : range(sim_params['msd']+N_vp+1, sim_params['msd']+2*N_vp+1)})
  # NEST does not support setting per-process seeds directly. Use global rng_seed instead.
...
def col_spike_detector(layer_gids_inside, layer_name, ignore_time, params = {"withgid": True, "withtime": True, "to_file": True}):
    ...
    detector = nest.Create( "spike_recorder", params= params )
    log.debug("CONNECT %s %s", layer_gids_inside, detector)
    nest.Connect( pre=layer_gids_inside, post=detector)

    return detector

def spike_figure(detector, region_name):
    plt.figure(figsize=(10, 6))

    for layer, recorder in detector.items():
        events = recorder.get("events")
        senders = events["senders"]
        ts = events["times"]
        plt.plot(ts, senders, ".", label=layer)  

    plt.title(f"{region_name} Spike Events")
    plt.xlabel("Time (ms)")
    plt.ylabel("Neuron ID")
    plt.legend(loc="upper right") 

    output_path = f'log/{region_name} Spike.png' 
    plt.savefig(output_path, format="png", dpi=300)  
    print(f"Figure saved to {output_path}")
    plt.close()  


def spike_Layer_figure(detector, region_name, layer_name):
    plt.figure(figsize=(10, 6))

    if layer_name in detector:
        recorder = detector[layer_name]
        events = recorder.get("events")
        senders = events["senders"]
        ts = events["times"]
        plt.plot(ts, senders, ".", label=layer_name)

        plt.title(f"{region_name} - {layer_name} Spike Events")
        plt.xlabel("Time (ms)")
        plt.ylabel("Neuron ID")
        plt.legend(loc="upper right")

        output_path = f'log/{region_name}_{layer_name}_Spike.png'
        plt.savefig(output_path, format="png", dpi=300)
        print(f"Figure saved to {output_path}")
    else:
        print(f"Layer '{layer_name}' not found in detector.")
        
    plt.close()



def spike_cloumn_figure(detector, region_name, layer_name, target_neurons):
    plt.figure(figsize=(10, 6))
    ...
    if layer_name in detector:
        ...
        mask = np.isin(senders, target_neurons)
        filtered_senders = senders[mask]
        filtered_ts = ts[mask]

        unique_ids = np.unique(filtered_senders)
        gid_to_index = {gid: idx for idx, gid in enumerate(unique_ids, start=1)}
        indexed_senders = np.array([gid_to_index[gid] for gid in filtered_senders])

        plt.plot(filtered_ts, indexed_senders, ".", label=layer_name)

        plt.yticks(ticks=range(1, len(unique_ids)+1), labels=unique_ids)  
        plt.title(f"{region_name} - {layer_name} Compact Spike Events")
        plt.xlabel("Time (ms)")
        plt.ylabel("Neuron (Original GID)")
        plt.legend(loc="upper right")
...
def get_connection_summary(region_params, layers, region):
    if region=='S1':
        ...
        # M1_internal_connection = np.load('ctx/M1_internal_connection.json')

        # M1_internal_connection = np.load('ctx/M1_internal_connection.pickle', allow_pickle= True)
        M1_internal_connection = np.load('ctx/'+ region_params['M1']['connection_info'])
        columns = ['pre_nodes_num', 'post_nodes_num', 'p_center', 'sigma','analy_conn_num', 'conn_num', 'weight_per_neuron', 'weight_per_connection']
        for post_l in layers.keys():
            rows_in = []
            cell_text_in = []
...
#-----------------------------------------------------------------------------------
# Create a neurons population for S1 or M1 (NEST 3.0)
#-----------------------------------------------------------------------------------
# def create_layers_ctx(layer_dep, layer_thickness, nbneuron, layer_size, scalefactor, element):
#     neuron_per_grid = math.pow((nbneuron / layer_thickness), 1.0 / 3)
#     Sub_Region_Architecture=[0, 0, 0]
#     Sub_Region_Architecture[0] = int(np.round(neuron_per_grid * layer_size[0] * scalefactor[0]))
#     Sub_Region_Architecture[1] = int(np.round(neuron_per_grid * layer_size[1] * scalefactor[1]))
#     Sub_Region_Architecture[2] = int(np.round(neuron_per_grid * layer_thickness))
#     layer = nest.Create(element, positions=nest.spatial.grid(shape=[Sub_Region_Architecture[0], Sub_Region_Architecture[1], Sub_Region_Architecture[2]], center=[0., 0., layer_dep/2], extent=[1.*scalefactor[0], 1.*scalefactor[1], 2.], edge_wrap=True))
#     print(Sub_Region_Architecture)
#     print(layer_dep)
#     print('created %d neurons' % len(layer))
#     return layer
...
def create_layers_ctx(layer_dep, layer_thickness, nbneuron, layer_size, scalefactor, element):
    ...
    # z_center = (layer_dep+(layer_thickness/2)) - (layer_size[2]/2)
    # position =nest.spatial.grid(shape=[Sub_Region_Architecture[0], Sub_Region_Architecture[1], Sub_Region_Architecture[2]], center=[0., 0., z_center], extent=[1.*scalefactor[0], 1.*scalefactor[1], layer_thickness], edge_wrap=True)
    # layer = nest.Create(element, positions= position)

    layer = nest.Create(element, positions=nest.spatial.grid(shape=[Sub_Region_Architecture[0], Sub_Region_Architecture[1], Sub_Region_Architecture[2]], center=[0., 0., 0], extent=[1.*scalefactor[0], 1.*scalefactor[1], 2.], edge_wrap=True))
...
#-----------------------------------------------------------------------------------
# connect (internal regional connection) S1 and M1 nest 3
#-----------------------------------------------------------------------------------
def connect_layers_ctx(pre_SubSubRegion, post_SubSubRegion, conn_params):
    ...
    if weight_distribution == 'lognormal':
        ...
        #              }
        weight = nest.random.lognormal(mean=conn_params['weight'], std=1.0)
        #delay =  conn_params['delay']
    else:
        # conn_dict = {'rule': 'pairwise_bernoulli',
        #              'p': conn_params['p_center']*nest.spatial_distributions.gaussian2D(
        #                  nest.spatial.distance.x, nest.spatial.distance.y, std_x=sigma_x, std_y=sigma_y),
    ...
    if conn_params['p_center'] != 0.0 and sigma_x != 0.0 and conn_params['weight'] != 0.0:
        ...
        # nest.Connect(nest.PairwiseBernoulli(source = pre_SubSubRegion, target = post_SubSubRegion, p=conn_params['p_center']*nest.spatial_distributions.gaussian2D(
        #                  nest.spatial.distance.x, nest.spatial.distance.y, std_x=sigma_x, std_y=sigma_y), weight = weight, delay = conn_params['delay']), allow_autapses=False, allow_multapses=False )
...
#-----------------------------------------------------------------------------------
# Connect specific layers across brain regions: S1 -> M1 (NEST 3)
#-----------------------------------------------------------------------------------
def connect_specific_layers_ctx(pre_layer, post_layer, conn_params):
    sigma_x = conn_params['sigma']/1000.
    sigma_y = conn_params['sigma']/1000.
    #print (conn_params)
    weight_distribution=conn_params['weight_distribution']
    ...
    if conn_params['p_center'] != 0.0 and sigma_x != 0.0 and conn_params['weight'] != 0.0:
        ...
        conn_dict = {'rule': 'pairwise_bernoulli',
                    ...
                    #                'upper_right': [1., 1., 2.]}},
                    #  'mask': {'box':
                    #     {'lower_left': [-0.5, -0.5, -1.],
                    #     'upper_right': [0.5, 0.5, 1.]}},
                     'allow_autapses': False,
                     'allow_multapses': False
                     }
        syn_spec = {
        ...
        nest.Connect(pre_layer, post_layer, conn_dict, syn_spec)
        
def connect_specific_layers_ctx_uniform(pre_layer, post_layer, conn_params):
    sigma_x = conn_params['sigma']/1000.
    sigma_y = conn_params['sigma']/1000.
    #print (conn_params)
    weight_distribution=conn_params['weight_distribution']
...
def add_poisson_generator(layer_gid, n_type, layer_name, ignore_time, region):      #
    ...
    #                               params={'rate': 700.0, "start": float(ini_time)})  # , 'label': layer_name})
    #             nest.Connect(pre=PSG, post=layer_gid, syn_spec={'weight': 5.0, 'delay': 1.5})
    #         elif layer_name in ['S1_L3_Pyr']:
    #             PSG = nest.Create('poisson_generator', 1,
    #                               params={'rate': 650.0, "start": float(ini_time)})  # , 'label': layer_name})
    #             nest.Connect(pre=PSG, post=layer_gid, syn_spec={'weight': 5.0, 'delay': 1.5})
    ...
    #             PSG = nest.Create('poisson_generator', 1, params={'rate': 400.0, "start": float(
    #                 ini_time_ini)})  # , 'label': layer_name})
    #             nest.Connect(pre=PSG, post=layer_gid, syn_spec={'weight': 5.0, 'delay': 1.5})
    ...
    #             PSG = nest.Create('poisson_generator', 1, params={'rate': 750.0, "start": float(
    #                 ini_time_ini)})  # , 'label': layer_name})
    #             nest.Connect(pre=PSG, post=layer_gid, syn_spec={'weight': 5.0, 'delay': 1.5})
    ...
    #             PSG = nest.Create('poisson_generator', 1, params={'rate': 600., "start": float(
    #                 ini_time_ini)})  # , 'label': layer_name})
    #             nest.Connect(pre=PSG, post=layer_gid, syn_spec={'weight': 5.0, 'delay': 1.5})
    ...
    #             PSG = nest.Create('poisson_generator', 1, params={'rate': 650.0, "start": float(
    #                 ini_time_ini)})  # , 'label': layer_name})
    #             nest.Connect(pre=PSG, post=layer_gid, syn_spec={'weight': 5.0, 'delay': 1.5})
...
def add_poisson_generator_pop(gids, layer_name):
    ...
    if layer_name == 'S1_L4_Pyr':
        ...
        nest.Connect(pre=PSG_arrow, post=gids, syn_spec={'weight': 10.0, 'delay': 1.5})
    return PSG_arrow



################################################################
# add poisson generator for each column
#################################################################
def add_poisson_generator_column(gids, start_time, param):
    Psg = nest.Create('poisson_generator', 1, params = param) #, 'label': layer_name})
    Psg.set({"start": start_time})

    log.debug("CONNECT %s", "X"*100)
    nest.Connect(pre=Psg, post=gids, syn_spec={'weight': 5.0, 'delay': 1.5})
    return Psg
...
def get_columns_data(layer_name,circle_center,radius_small,my_area=None):
    ...
    circle_gids = []
    for i in np.arange(len(circle_center)):
        idx = np.where(np.linalg.norm([(gid_pos[:,0]-circle_center[i][0]),(gid_pos[:,1]-circle_center[i][1])],axis=0)<=radius_small)[0]
        print('number of neurons in channel ',str(i),'for ',layer_name,' : ',str(len(idx)))
        circle_gids.append([[int(x[3]),x[0:3].tolist()] for x in gid_pos[idx,:]])
...
def get_channel_centers(bg_params,hex_center=[0,0],ci=4,hex_radius=0.10):
    center_aux = []
...
def apply_direction_stimulus(ctx_bg_input,direction,time_start,time_stop):
    ...
    for j in np.arange(len(ctx_bg_input['M1'][0])): #for each circle of gids
        ...
        circle_gid_detector[str(j)] = nest.Create("spike_recorder", params=params)
        log.debug("CONNECT %s", "X"*100)
