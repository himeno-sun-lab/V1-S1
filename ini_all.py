...
def instantiate_ctx(ctx_params,scalefactor, initial_ignore, region_name):
  ...
  for l in range(len(Layer_Name)):
    ...
    for n_type in ctx_params[region_name]['neuro_info'][Layer_Name[l]].keys():
      ...
      if n_type_info['EorI'] == "E":
        SubSubRegion_Excitatory.append(nest_routine.create_layers_ctx( layer_depth[l], layer_thickness[l], n_type_info['Cellcount_mm2'], layer_size, scalefactor, neuronmodel))
        ...
        SubSubRegion_Inhibitory.append(nest_routine.create_layers_ctx(layer_depth[l], layer_thickness[l], n_type_info['Cellcount_mm2'], layer_size, scalefactor, neuronmodel))
  ...
  internal_connection = json.load(open('ctx/'+ ctx_params[region_name]['connection_info']['internal'], 'r', encoding='utf-8'))
  ...
  for pre_layer_name in ctx_layers.keys():
    for post_layer_name in ctx_layers.keys():
      ...
      # print(internal_connection[pre_layer_name][post_layer_name])
