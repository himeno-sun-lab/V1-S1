...
target_gids_inside, target_gids_outside = get_input_column_layers_ctx_M1(ctx_M1_layers, target_layer_name, target_layer_name,  radius)
...
detector_micro = {}
for layer_name in ctx_M1_layers.keys():
    circle_col_gids[layer_name] = get_micro_columns_data(ctx_M1_layers, target_layer_name, layer_name, micro_radius )
    layer_gids_inside[layer_name], layer_gids_outside[layer_name] = get_input_column_layers_ctx_M1(ctx_M1_layers, target_layer_name, layer_name, radius)
...
poisson_gens.append(layer_poisson_generator(circle_col_gids[target_layer_name][target_micro_col_num], target_layer_name))
################################
# 9) simulation
num_trials = 2
...
amp_changes_in_abs_norm = [x * 100 / amp_changes_in [target_layer_name] for x in amp_changes_in_abs]
amp_changes_out_abs_norm= [x * 100 / amp_changes_out [target_layer_name] for x in amp_changes_out_abs]

del amp_changes_in_abs_norm[layer_dic[target_layer_name]-1:layer_dic[target_layer_name]]
del amp_changes_out_abs_norm[layer_dic[target_layer_name]-1:layer_dic[target_layer_name]]
...
for n in range(len( circle_col_gids[layer_name])):
    ...
    save_results_to = '/home/morteza/Desktop/reports/191112 (micro_col_test)/'
    plt.savefig( save_results_to + target_layer_name+ ' stim inside col no' + str(n) + '.jpeg', bbox_inches='tight' )
