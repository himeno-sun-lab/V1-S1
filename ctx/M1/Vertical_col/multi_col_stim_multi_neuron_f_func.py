...
# -*- coding: utf-8 -*-
########################################################################################################################
# In this script I design seven macro col and seven micro column to see relation among the the micro columns
...
def get_macro_channel_centers(bg_params,hex_center=[0,0],ci=7,hex_radius=0.240):
    ...
    return center_aux
###############################################################
def get_micro_channel_centers(macro_channel_centers,ci=7,hex_radius=0.16/3.):
    micro_center_aux = [[], [], [], [], [], [], []]
    for i in range(len(macro_channel_centers)):
        center_aux=[]
        #print(macro_channel_centers [i] )
        ...
        for j in np.arange(ci-1):
            ...
            center_aux.append(x_y)
            micro_center_aux[i] =center_aux
        print('micro_circle' +str(i)+ 'are:', micro_center_aux)
    return micro_center_aux
###############################################################
def get_input_column_layers_ctx_M1(ctx_M1_layers, target_layer_name, layer_name, circle_centers, radius):
    ...
    for i in range(len(circle_centers)):
        for nn in range(len(layer_gids)):
            if np.linalg.norm([(neuron_positions[nn][0] - circle_centers[i][0]), (neuron_positions[nn][1] - circle_centers[i][1])]) <= radius:
                inside_gids[i].append(layer_gids[nn])
                inside_poses[i].append( neuron_positions [nn] )
            else:
                outside_gids[i].append(layer_gids[nn])
        if len(inside_poses[i]) >= 1:
            np.savetxt( './log/' + layer_name + '_macro_column' + str(i) + '.txt', inside_poses[i], fmt='%.3f' )
        if layer_name == target_layer_name:
            print ('all', layer_name ,'neuron numbers were: ', len(layer_gids), 'and only', len(inside_gids[i]), 'neurons were stimulated inside the column using poisson generator....')

    return inside_gids , outside_gids,

################################################################
def get_micro_columns_data(ctx_M1_layers,target_layer_name, layer_name, micro_circle_centers, micro_radius):
    #circle_center = [[0.0, 0.0], [0.11*np.cos(np.pi/6.), 0.11*np.sin(np.pi/6.)], [0.0, 0.11], [-0.11*np.cos(np.pi/6.), 0.11*np.sin(np.pi/6.)],
                     #[-0.11*np.cos(np.pi/6.), -0.11*np.sin(np.pi/6.)], [0.0, -0.11], [0.11*np.cos(np.pi/6.), -0.11*np.sin(np.pi/6.)] ]
    print( 'micro_circle_centers:', micro_circle_centers)
    print(len(micro_circle_centers) )
    print( len( micro_circle_centers[0] ) )
    circle_gids = [[], [], [], [], [], [], []]
    circle_poses = [[], [], [], [], [], [], []]
    for k in range(len(micro_circle_centers)):
        circle_gids[k] = [[], [], [], [], [], [], []]
        circle_poses[k] = [[], [], [], [], [], [], []]
        i=0
        for circle_center in micro_circle_centers[k]:
            print(circle_center)
            layer_gids = nest.GetNodes( ctx_M1_layers [layer_name] ) [0]
            neuron_positions = ntop.GetPosition( layer_gids )
            for nn in range(len(layer_gids)):
                if np.linalg.norm([(neuron_positions[nn][0] - circle_center[0]), (neuron_positions[nn][1] - circle_center[1])]) <= micro_radius:
                    circle_gids[k][i].append(layer_gids[nn])
                    circle_poses[k][i].append(neuron_positions[nn])

            print( 'number of neurons in macro_col', str(k), 'and micro_col', str( i ), 'for ', layer_name, ' : ', str( len( circle_gids[k][i] ) ) )
            if len(circle_poses[k][i] ) >= 1:
                np.savetxt( './log/' + layer_name +'_micro_column ' + str(k) + '_micro_column ' + str(i) + '.txt', circle_poses[k][i], fmt='%.3f' )
            i+=1
...
def layer_poisson_generator(layer_gid, layer_name, params={"withgid": True, "withtime": True, "to_file": False}):
    ...
    if layer_name == 'M1_L23_CC':
        PSG_arrow = nest.Create('poisson_generator', 1, params={'rate': 200.0}) #, 'label': layer_name})
        nest.Connect(pre=PSG_arrow, post=layer_gid, syn_spec={'weight': 10.0, 'delay': 1.5})
    elif layer_name == 'M1_L23_VIP':
        PSG_arrow = nest.Create('poisson_generator', 1, params={'rate': 50.0}) #, 'label': layer_name})
        nest.Connect(pre=PSG_arrow, post=layer_gid, syn_spec={'weight': 10.0, 'delay': 1.5})
    elif layer_name == 'M1_L5B_PT':
        PSG_arrow = nest.Create('poisson_generator', 1, params={'rate': 100.0}) #, 'label': layer_name})
        nest.Connect(pre=PSG_arrow, post=layer_gid, syn_spec={'weight': 10.0, 'delay': 1.5})
...
def layer_spike_detector(layer_gids_inside, layer_gids_outside, layer_name, record_onset, ignore_time, ran_inside_neurons, ran_outside_neurons, params = {"withgid": True, "withtime": True, "to_file": False}):
    ...
    # just to have record of all neurons activity in a file
    detector_all = nest.Create( "spike_detector", params={"withgid": True, "withtime": True, "to_file": True, 'label': layer_name, "start": float(record_onset)})
    nest.Connect( pre=layer_gids_inside + layer_gids_outside, post=detector_all )
...
###############################################################
def micro_col_spike_detector(circle_gids, layer_name, record_onset, params = {"withgid": True, "withtime": True, "to_file": False}):
    detector_micro = [[],[],[],[],[],[],[]]
    for i in range(len(circle_gids)):
        print( 'spike detector for ' + layer_name + 'in circle number' + str(i) )
        params.update( {'label': layer_name, "start": float(record_onset)} )

        # Add detector for all neuron types
        detector_micro[i] = nest.Create( "spike_detector", params=params )
        nest.Connect( pre=circle_gids[i], post=detector_micro[i] )

    return detector_micro
################################################################
...
#########################################################
def calculate_membrane_potential_and_conductance(multimeter,layer_name, neurons_num, num_trials, Nest_time_steps):
    events = {}
    for num in range(neurons_num):
        events [num] = nest.GetStatus(multimeter[layer_name][num])[0] ["events"]

    times = ((events [0] ['times']) [0:int(record_intervals*3*(1/Nest_time_steps))])

    if neurons_num > 1:
        conductance_inhibitory, conductance_excitatory, membrane_potantial= average_recorders_outputs( events, neurons_num)  # average among the recorded neurons
    else:
        conductance_inhibitory = events [0] ['g_in']
        conductance_excitatory= events [0] ['g_ex']
        membrane_potantial = events [0] ['V_m']

    membrane_potantial_ave  = average_over_trials( num_trials, membrane_potantial) #, sim_params ['simDuration'], sim_params ['dt'])
    conductance_inhibitory_ave = average_over_trials( num_trials, conductance_inhibitory) #, sim_params ['simDuration'], sim_params ['dt'] )
    conductance_excitatory_ave = average_over_trials( num_trials, conductance_excitatory) #, sim_params ['simDuration'], sim_params ['dt'] )

    return conductance_inhibitory_ave, conductance_excitatory_ave, membrane_potantial_ave, times

###############################################################
def calculate_frequency_sem_sd (detector, layer_name, num_trials, layer_gids, bin_list, flag):
    spike_t = nest.GetStatus(detector[layer_name])[0]['events']['times']    # recorded from all neurons inside
    freq_ave = calcualte_ave_sem_sd( num_trials, spike_sort_count_freq(num_trials, spike_t, sim_params ['initial_ignore'], sim_params ['simDuration'], layer_gids, bin_list )[int(float(flag))]) [0]
    freq_sem = calcualte_ave_sem_sd( num_trials, spike_sort_count_freq(num_trials, spike_t, sim_params ['initial_ignore'], sim_params ['simDuration'], layer_gids, bin_list )[int(float(flag))] ) [1]
    freq_sd = calcualte_ave_sem_sd( num_trials, spike_sort_count_freq(num_trials, spike_t, sim_params ['initial_ignore'], sim_params ['simDuration'], layer_gids, bin_list )[int(float(flag))] ) [2]

    return freq_ave, freq_sem, freq_sd
    ...
    if hapc.Rank()==0:
        ...
        if len(pop[layer_name])>0:
            ...
            if len(pop[layer_name])>0:
                ...
                return macro_columns_activity, macro_columns_spikes[0][:, 1]
...
radius = 0.16
micro_radius = 0.16/3.
target_macro_col_num = 0
target_micro_col_num = 0
###############################
# calculating centers of micro and macro columns
macro_circle_center_params = get_macro_channel_centers(sim_params, hex_center=[0, 0],ci=sim_params['macro_columns_nb'], hex_radius=sim_params['hex_radius'])
micro_circle_center_params = get_micro_channel_centers(macro_circle_center_params,ci=sim_params['micro_columns_nb'], hex_radius=sim_params['channels_radius']*2./3.)
# 7) selecting the VIP neurons for the stimulation located inside the coloumn
poisson_gens = []    # in case of one trial     poisson_gens = {}
target_layer_name = {'1':'M1_L23_CC','2':'M1_L23_VIP','3':'M1_L5B_PT'}
target_gids_inside, target_gids_outside = get_input_column_layers_ctx_M1(ctx_M1_layers, target_layer_name['3'], target_layer_name['3'], macro_circle_center_params,  radius)
...
poisson_gens.append(layer_poisson_generator(target_gids_inside[target_macro_col_num], target_layer_name['3']))
...
detector_micro = {}
for layer_name in ctx_M1_layers.keys():
    circle_col_gids[layer_name] = get_micro_columns_data(ctx_M1_layers, target_layer_name['1'], layer_name, micro_circle_center_params, micro_radius )
    layer_gids_inside[layer_name], layer_gids_outside[layer_name] = get_input_column_layers_ctx_M1(ctx_M1_layers, target_layer_name['1'], layer_name,macro_circle_center_params, radius)
    detectors_in[layer_name], detectors_out[layer_name], multimeters_in[layer_name], multimeters_out[layer_name], voltmeters[layer_name] = layer_spike_detector(layer_gids_inside[layer_name][target_macro_col_num], layer_gids_outside[layer_name][target_macro_col_num],
                                                                                                                                                               layer_name, record_onset, sim_params['initial_ignore'],
                                                                                                                                                                inside_neurons_torecord, outside_neurons_torecord)

    detector_micro [layer_name] = micro_col_spike_detector( circle_col_gids [layer_name][target_macro_col_num], layer_name, record_onset )
    #for i in range(7):
        #layer_gids_all [layer_name][i] = layer_gids_inside [layer_name][i] + layer_gids_outside [layer_name][i]

poisson_gens.append(layer_poisson_generator(circle_col_gids[target_layer_name['1']][target_macro_col_num][target_micro_col_num], target_layer_name['1']))
#poisson_gens.append(layer_poisson_generator(circle_col_gids[target_layer_name['2']][target_macro_col_num][target_micro_col_num], target_layer_name['2']))
target_gids_inside, target_gids_outside = get_input_column_layers_ctx_M1(ctx_M1_layers, target_layer_name['2'], target_layer_name['2'], macro_circle_center_params,  radius)
poisson_gens.append(layer_poisson_generator(target_gids_inside[target_macro_col_num], target_layer_name['2']))
################################
# 9) simulation
num_trials = 2
...
for n in range(num_trials):
    if n == 0:
        print(n)
        nest.SetStatus( poisson_gens [0], {'origin': nest.GetKernelStatus() ['time'], "start": (num_trials*simulation_time) + initial_ignore + record_intervals, 'stop': (num_trials*simulation_time) + initial_ignore +(2*record_intervals)} )
        nest.SetStatus( poisson_gens [1], {'origin': nest.GetKernelStatus() ['time'], "start": (num_trials * simulation_time) + initial_ignore + record_intervals, 'stop': (num_trials * simulation_time) + initial_ignore + (2 * record_intervals)} )
        nest.SetStatus( poisson_gens [2], {'origin': nest.GetKernelStatus() ['time'], "start": (num_trials * simulation_time) + initial_ignore + record_intervals, 'stop': (num_trials * simulation_time) + initial_ignore + (2 * record_intervals)} )
        ...
        nest.SetStatus( poisson_gens [2], {'origin': nest.GetKernelStatus() ['time'], "start": record_intervals, 'stop': 2 * record_intervals} )
...
g_exs_out_ave = {}
for layer_name in ctx_M1_layers.keys():
    g_ins_in_ave [layer_name], g_exs_in_ave [layer_name], mps_in_ave [layer_name], times = calculate_membrane_potential_and_conductance(multimeters_in, layer_name, inside_neurons_torecord, num_trials, float(sim_params['dt']))[0:4]
    g_ins_out_ave [layer_name], g_exs_out_ave [layer_name], mps_out_ave [layer_name] = calculate_membrane_potential_and_conductance(multimeters_out, layer_name, outside_neurons_torecord, num_trials, float(sim_params['dt']))[0:3]
...
freq_out_rest_sd ={}
for layer_name in ctx_M1_layers.keys():
    freq_in_ave [layer_name], freq_in_sem [layer_name], freq_in_sd [layer_name]= calculate_frequency_sem_sd (detectors_in, layer_name, num_trials, layer_gids_inside[layer_name][target_macro_col_num], bin_list, flag = '0')
    freq_out_ave [layer_name], freq_out_sem [layer_name], freq_out_sd [layer_name] = calculate_frequency_sem_sd(detectors_out, layer_name, num_trials, layer_gids_outside [layer_name][target_macro_col_num], bin_list, flag = '0' )
    freq_in_rest_ave [layer_name], freq_in_rest_sem [layer_name], freq_in_rest_sd [layer_name] = calculate_frequency_sem_sd(detectors_in, layer_name, num_trials, layer_gids_inside [layer_name][target_macro_col_num], bin_list,flag = '1' )
    freq_out_rest_ave [layer_name], freq_out_rest_sem [layer_name], freq_out_rest_sd [layer_name] = calculate_frequency_sem_sd(detectors_out, layer_name, num_trials, layer_gids_outside [layer_name][target_macro_col_num], bin_list, flag = '1' )

spike_t_all_from_gdfs = {}
spike_t_in = {}
spike_t_micro = {}
end_time = record_onset + num_trials *2*3*record_intervals
for layer_name in ctx_M1_layers.keys():
    spike_t_micro [layer_name] = [[],[],[],[],[],[],[]]
    spike_t_all_from_gdfs[layer_name] = get_firing_rate_macro_column(layer_name, layer_gids_inside[layer_name], record_onset, end_time)[1]
    spike_t_in [layer_name] = nest.GetStatus( detectors_in [layer_name] ) [0] ['events'] ['times']
    for n in range(7):
        spike_t_micro [layer_name][n] = nest.GetStatus( detector_micro [layer_name][n] ) [0] ['events'] ['times']

layer_name = 'M1_L1_ENGC'
print(spike_t_all_from_gdfs[layer_name])
print(spike_t_all_from_gdfs[layer_name][n])
print(spike_t_micro [layer_name])
print(spike_t_micro [layer_name][n])
############################################################
freq_micro_col = {}
freq_micro_col_rest = {}
for layer_name in ctx_M1_layers.keys():
    freq_micro_col [layer_name] = []
    freq_micro_col_rest [layer_name] = []
    for n in range(len( circle_col_gids[layer_name][target_macro_col_num])):
        freq_micro_col[layer_name].append([])
        freq_micro_col_rest [layer_name].append ([])

    for n in range( len( circle_col_gids [layer_name][target_macro_col_num])):
        freq_micro_col [layer_name][n] = spike_sort_count_freq( num_trials, spike_t_micro[layer_name][n], sim_params ['initial_ignore'], sim_params ['simDuration'], circle_col_gids[layer_name][target_macro_col_num][n], bin_list ) [0]
        freq_micro_col_rest [layer_name] [n] = spike_sort_count_freq( num_trials, spike_t_micro[layer_name] [n], sim_params ['initial_ignore'], sim_params ['simDuration'], circle_col_gids [layer_name][target_macro_col_num][n], bin_list ) [1]

freq_micro_col_ave = {}
freq_micro_col_sem = {}
freq_micro_col_sd = {}

freq_micro_col_rest_ave = {}
freq_micro_col_rest_sem = {}
freq_micro_col_rest_sd = {}
for layer_name in ctx_M1_layers.keys():
    freq_micro_col_ave [layer_name] = []
    freq_micro_col_sem [layer_name] = []
    freq_micro_col_sd [layer_name] = []
    freq_micro_col_rest_ave [layer_name] = []
    freq_micro_col_rest_sem [layer_name] = []
    freq_micro_col_rest_sd [layer_name] = []
    for n in range( len( circle_col_gids [layer_name][target_macro_col_num]) ):
        freq_micro_col_ave [layer_name].append( [] )
        freq_micro_col_sem [layer_name].append( [] )
        freq_micro_col_sd [layer_name].append( [] )
        freq_micro_col_rest_ave [layer_name].append( [] )
        freq_micro_col_rest_sem [layer_name].append( [] )
        freq_micro_col_rest_sd [layer_name].append( [] )

    for n in range( len( circle_col_gids [layer_name][target_macro_col_num]) ):
        freq_micro_col_ave [layer_name] [n] = calcualte_ave_sem_sd( num_trials, freq_micro_col [layer_name] [n] ) [0]
        freq_micro_col_sem [layer_name] [n] = calcualte_ave_sem_sd( num_trials, freq_micro_col [layer_name] [n] ) [1]
        freq_micro_col_sd [layer_name] [n] = calcualte_ave_sem_sd( num_trials, freq_micro_col [layer_name] [n] ) [2]

        freq_micro_col_rest_ave [layer_name] [n] = calcualte_ave_sem_sd( num_trials, freq_micro_col_rest [layer_name] [n] ) [0]
        freq_micro_col_rest_sem [layer_name] [n] = calcualte_ave_sem_sd( num_trials, freq_micro_col_rest [layer_name] [n] ) [1]
        freq_micro_col_rest_sd [layer_name] [n] = calcualte_ave_sem_sd( num_trials, freq_micro_col_rest [layer_name] [n] ) [2]
#######################################################################################################################
...
amp_changes_micro_col = {}
amp_changes_micro_col_abs={}
for n in range( len( circle_col_gids [layer_name][target_macro_col_num] ) ):
    amp_changes_micro_col_abs[n] = []
...
for layer_name in ctx_M1_layers.keys():
    ...
    freq_micro_col_rest_sem [layer_name] = []
    freq_micro_col_rest_sd [layer_name] = []
    amp_changes_micro_col [layer_name] = []
    for n in range( len( circle_col_gids [layer_name][target_macro_col_num] ) ):
        freq_micro_col_ave [layer_name].append( [] )
        freq_micro_col_sem [layer_name].append( [] )
        freq_micro_col_sd [layer_name].append( [] )
        ...
        freq_micro_col_rest_sem [layer_name].append( [] )
        freq_micro_col_rest_sd [layer_name].append( [] )
        amp_changes_micro_col [layer_name].append( [] )
    ...
    for n in range( len( circle_col_gids [layer_name][target_macro_col_num] ) ):
        ...
        amp_changes_micro_col [layer_name] [n] = np.average( freq_micro_col_ave [layer_name] [n] [25:50] ) - np.average(freq_micro_col_rest_ave [layer_name] [n] [25:50] )
        amp_changes_micro_col_abs [n].append( amp_changes_micro_col [layer_name] [n] )
...
plt.savefig( save_results_to + target_layer_name+ ' stim inside' + '.jpeg', bbox_inches='tight' )
...
save_results_to = '/home/morteza/Desktop/reports/191031 (final M1 tuning)/'
plt.savefig( save_results_to + target_layer_name+ ' stim outside' + '.jpeg', bbox_inches='tight' )

########################################################################################################################
# try to make graphs of all rasters at once for micro columns
# graph for the selective raster plot of all pop in one graph #
del bin_list[-1]
plt.rcParams ["figure.figsize"] = [8, 64]
plt.rc('xtick', labelsize=20)

for n in range(len( circle_col_gids[layer_name])):
    i = 1
    #ax = plt.subplot( 19, 1, i )
    figure, ax = plt.subplots()
    for layer_name in ctx_M1_layers.keys():
        ...
        ax.tick_params( axis='both', length=5.0, width=2.0 )
        if i == 1:
            plt.title('inside response of col ' + str(n) + f' to stimulation of {target_layer_name} col ' + str(target_micro_col_num) + '\n', fontsize=15)
        if i == 19:
            plt.xlabel( "time (ms)", fontsize=15 )
        else:
            plt.xticks( [] )
        ...
        ax.plot(bin_list, freq_micro_col_ave[layer_name][n], color='g', linewidth=3.0)
        plt.fill_between(bin_list, freq_micro_col_ave[layer_name][n] - freq_micro_col_sem[layer_name][n],  freq_micro_col_ave[layer_name][n] + freq_micro_col_sem[layer_name][n], facecolor='lightgreen', alpha= 0.4)
        ax.plot(bin_list, freq_micro_col_rest_ave[layer_name][n],color='b', linewidth=3.0)
        plt.fill_between(bin_list, freq_micro_col_rest_ave[layer_name][n] - freq_micro_col_rest_sem[layer_name][n],  freq_micro_col_rest_ave[layer_name][n] + freq_micro_col_rest_sem[layer_name][n], facecolor='skyblue', alpha= 0.4)
        i += 1
    
    save_results_to = '/home/morteza/Desktop/reports/191114 (micro_col_2pop_stim)/'
    plt.savefig( save_results_to + target_layer_name['1'] + ' and '+ target_layer_name['2'] + ' stim inside col no' + str(n) + '.jpeg', bbox_inches='tight' )
...
'''
# plot changes of amplitude for stimulations of different micro columns
...
for n in range(len( circle_col_gids[layer_name])):
    ...
    plt.plot(x, amp_changes_micro_col_abs[n], lw=2, marker = 'o', markersize=14, label= 'col_%s' %n)#(rad,))
    plt.xlabel("\nNeuron types")
    plt.ylabel("relative change in firing rate (Hz)")
    plt.legend(loc='best')
    plt.title('Response to micro stim' + target_layer_name['1']+ ', '+ target_layer_name['2'] + ' and macro stim '+ target_layer_name['3'] + ' inside col_' +str(target_micro_col_num) , fontsize = 30)
    save_results_to = '/home/morteza/Desktop/reports/191206 (3 consecuative input)/one micro columns/L23_CC micro & L23_VIP & L5B_PT macro/'
    plt.savefig( save_results_to + target_layer_name['1'] + ' and '+ target_layer_name['2'] + ' and '+ target_layer_name['3'] + ' stim inside col no' + str(target_micro_col_num) + '.jpeg')
