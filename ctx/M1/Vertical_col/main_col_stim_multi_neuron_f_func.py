...
def calculate_membrane_potential_and_conductance(multimeter,layer_name, neurons_num, num_trials, Nest_time_steps):
    ...
    membrane_potantial_ave  = average_over_trials( num_trials, membrane_potantial)
    conductance_inhibitory_ave = average_over_trials( num_trials, conductance_inhibitory )
    conductance_excitatory_ave = average_over_trials( num_trials, conductance_excitatory )
    ...
    if hapc.Rank()==0:
        ...
        if len(pop[layer_name])>0:
            ...
            if len(pop[layer_name])>0:
                ...
                return macro_columns_activity, macro_columns_spikes
...
inside_neurons_torecord =10
outside_neurons_torecord = 10
multimeters_in = {}
multimeters_out = {}
voltmeters = {}
detectors_in = {}
...
freq_out_rest_sd ={}
for layer_name in ctx_M1_layers.keys():
    freq_in_ave [layer_name], freq_in_sem [layer_name], freq_in_sd [layer_name]= calculate_frequency_sem_sd (detectors_in, layer_name, num_trials, layer_gids_inside[layer_name], bin_list, flag = '0')
    freq_out_ave [layer_name], freq_out_sem [layer_name], freq_out_sd [layer_name] = calculate_frequency_sem_sd(detectors_out, layer_name, num_trials, layer_gids_outside [layer_name], bin_list, flag = '0' )
    freq_in_rest_ave [layer_name], freq_in_rest_sem [layer_name], freq_in_rest_sd [layer_name] = calculate_frequency_sem_sd(detectors_in, layer_name, num_trials, layer_gids_inside [layer_name], bin_list,flag = '1' )
    freq_out_rest_ave [layer_name], freq_out_rest_sem [layer_name], freq_out_rest_sd [layer_name] = calculate_frequency_sem_sd(detectors_out, layer_name, num_trials, layer_gids_outside [layer_name], bin_list, flag = '1' )

spike_t_all_from_gdfs = {}
spike_t_in = {}
end_time = record_onset + num_trials *2*3*record_intervals
layer_gids_inside_all=[[],[],[],[],[],[],[]]
layer_name = 'M1_L1_ENGC'
for t in range( 7 ):
    layer_gids_inside_all [t] = layer_gids_inside [layer_name]
spike_t_all_from_gdfs[layer_name] = get_firing_rate_macro_column(layer_name, layer_gids_inside_all, record_onset, end_time)[1]
for layer_name in ctx_M1_layers.keys():
