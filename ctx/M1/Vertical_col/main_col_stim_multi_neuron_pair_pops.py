...
def layer_poisson_generator(layer_gid, layer_name, params={"withgid": True, "withtime": True, "to_file": False}):
    ...
    print('input layer_name is:', layer_name)
    # stimulation by using PSG
    if layer_name == 'M1_L23_VIP':
        PSG_arrow = nest.Create('poisson_generator', 1, params={'rate':  1000.0}) #, 'label': layer_name})
        nest.Connect(pre=PSG_arrow, post=layer_gid, syn_spec={'weight': 10.0, 'delay': 1.5})

    if layer_name == 'M1_L23_VIP_inactivated':
...
def spike_sort_count_freq(num_trials, spike_t, initial_ignore, simulation_time, layer_gids, bin_list):
    ...
    return freq_trials, freq_rest_trials, spike_t_trials, spike_t_rest_trials

############################
def overlap_trials(eve, t, num_trials, initial_ignore, simulation_time):
    for n in range( num_trials ):
        for index, tt in enumerate(t):
            while t[index] < t[index+1]:
                if tt > ((num_trials+n) * simulation_time) + initial_ignore:
                    start_point = index
                    for index, tt in enumerate(t):
                        if tt > ((num_trials+1+n) * simulation_time) + initial_ignore:
                            end_point = index
                            if n==0:
                                t_overlap = t[start_point, end_point]- ((num_trials * simulation_time) + initial_ignore)
                                eve_overlap = eve [start_point, end_point]
                            else:
                                t[start_point, end_point] = t[start_point, end_point] - (((num_trials + n) * simulation_time) + initial_ignore)
                                t_overlap = np.concatenate((t_overlap, t[start_point, end_point]))
                                eve_overlap = np.concatenate((eve_overlap, eve[start_point, end_point]))
                break

    return eve_overlap, t_overlap
...
poisson_gens.append(layer_poisson_generator(target_gids_inside, target_layer_name))     # poisson_gens [0]

# make a PSG to inhibit the activated VIP
target_inac_layer_name = 'M1_L23_VIP_inactivated'
poisson_gens.append(layer_poisson_generator(target_gids_inside, target_inac_layer_name))     # poisson_gens [1]
###############################
...
# This part try to excite the one excitatoty neuron and check the effect of VIP inhibition
exc_layer_name = 'M1_L5A_CS'
poisson_gens.append(layer_poisson_generator(layer_gids_inside[exc_layer_name], exc_layer_name))   # poisson_gens [2]
inh_layer_name = 'M1_L5A_SST'
poisson_gens.append(layer_poisson_generator(layer_gids_inside[inh_layer_name], inh_layer_name))   # poisson_gens [3]
################################
# 9) simulation
num_trials = 30
...
for n in range(num_trials):
    if n == 0:
        ...
        print(n)
        nest.SetStatus( poisson_gens [0], {'origin': nest.GetKernelStatus() ['time'], "start": (num_trials*simulation_time) + (2*record_intervals), 'stop': (num_trials*simulation_time) + (2*record_intervals) + 200.} )
        nest.SetStatus( poisson_gens [1], {'origin': nest.GetKernelStatus() ['time'], "start": (num_trials*simulation_time) + (2*record_intervals) + 200., 'stop': (num_trials*simulation_time) + (3*record_intervals)} )
        ...
        nest.SetStatus( poisson_gens [3], {'origin': nest.GetKernelStatus() ['time'], "start": (num_trials*simulation_time) + (2*record_intervals), 'stop': (num_trials*simulation_time) + (3*record_intervals)} )
        nest.Simulate( (num_trials*simulation_time) + initial_ignore + simulation_time )
    elif n == num_trials -1:
        print(n)
        nest.SetStatus( poisson_gens [0], {'origin': nest.GetKernelStatus() ['time'], "start": record_intervals, 'stop': record_intervals + 200.0} )
        nest.SetStatus( poisson_gens [1], {'origin': nest.GetKernelStatus() ['time'], "start": record_intervals + 200.0, 'stop': 2*record_intervals} )
        nest.SetStatus( poisson_gens [2], {'origin': nest.GetKernelStatus() ['time'], "start": record_intervals, 'stop': 2*record_intervals} )
        nest.SetStatus( poisson_gens [3], {'origin': nest.GetKernelStatus() ['time'], "start": record_intervals, 'stop': 2*record_intervals} )
...
for layer_name in ctx_M1_layers.keys():
    ...
    spike_in_accum[layer_name] = spike_sort_count_freq(num_trials, spike_t_in[layer_name], sim_params['initial_ignore'], sim_params['simDuration'], layer_gids_inside[layer_name], bin_list)[2]
    spike_out_accum[layer_name] = spike_sort_count_freq( num_trials, spike_t_out [layer_name], sim_params ['initial_ignore'], sim_params ['simDuration'], layer_gids_outside [layer_name], bin_list ) [2]
    spike_in_rest_accum[layer_name] = spike_sort_count_freq( num_trials, spike_t_in [layer_name], sim_params ['initial_ignore'], sim_params ['simDuration'], layer_gids_inside [layer_name], bin_list ) [3]
    spike_out_rest_accum[layer_name] = spike_sort_count_freq( num_trials, spike_t_out [layer_name], sim_params ['initial_ignore'], sim_params ['simDuration'], layer_gids_outside [layer_name], bin_list ) [3]
...
########################################################################################################################
#making raster plots of different populations
########################################################################################################################
de = {}
eve = {}
t = {}
...
n = 0
for layer_name in ctx_M1_layers.keys():
    de [layer_name] = nest.GetStatus( detectors_in [layer_name], keys="events" ) [0]
    eve [layer_name] = de [layer_name] ["senders"]
    t [layer_name] = de [layer_name] ["times"]

    # to make an order neuron number for the raster plot
    eve_s [layer_name] = [None] * len( eve [layer_name] )
    for m in range( len( eve [layer_name] ) ):
        if eve [layer_name] [m] > 10000:
            tar = eve [layer_name] [m]
            j = j + 1
            eve_s [m] = j
            eve [layer_name] [m] = j
            for k in range( len( eve [layer_name] ) ):
                if eve [layer_name] [k] == tar:
                    eve_s [k] = j
                    eve [layer_name] [k] = j

    eve_all = np.concatenate( (eve_all, eve [layer_name]), axis=None )
    t_all = np.concatenate( (t_all, t [layer_name]), axis=None )

###############################################################
# overlap all excited trials to accumulate spike raster of all trials in one trial
###############################################################
'''
eve_overlap = {}
t_overlap = {}
for layer_name in ctx_M1_layers.keys():
    eve_overlap[layer_name], t_overlap[layer_name] =  overlap_trials(eve[layer_name], t[layer_name], num_trials, sim_params['initial_ignore'], sim_params['simDuration'])
'''
###############
...
for layer_name in ctx_M1_layers.keys():
  ...
  save_results_to = '/home/morteza/Desktop/reports/191001/'
  plt.savefig(save_results_to + str(i) +'.jpeg')  
  i=i+1
...
i=1
colors = ['lightcoral', 'lightcoral', 'royalblue', 'lightcoral', 'lightcoral', 'lightcoral', 'royalblue', 'royalblue', 'royalblue', 'lightcoral', 'lightcoral', 'royalblue', 'royalblue', 'royalblue', 'lightcoral', 'lightcoral', 'royalblue', 'lightcoral', 'lightcoral']  
for layer_name in ctx_M1_layers.keys():
    ax = plt.subplot(19, 1, i)
    ax.tick_params(axis='both',length = 5.0,width = 2.0)
    if i ==1:
        plt.title('M1_activity\n')
    if i==19:
        plt.xlabel("time (ms)")
    else:
        plt.xticks([])
    ...
    #ax1=ax.twinx()
    #plt.yticks([])
    #plt.ylabel('F='+ str(rate[layer_name]) +'Hz',fontsize = 18, rotation = 0, labelpad=60)
    
    ## to remove to graph borders##
    #for spine in plt.gca().spines.values():
        #spine.set_visible(False)
    plt.plot(t[layer_name], eve[layer_name], "|", color=colors[i-1])
    i =i+1 
    
save_results_to = '/home/morteza/Desktop/reports/191001/'
plt.savefig(save_results_to + 'M1_activity' +'.jpeg', bbox_inches='tight')

########################################################################################################################
# only for selected pops
i=1
colors = ['lightcoral', 'lightcoral', 'royalblue', 'lightcoral', 'lightcoral', 'lightcoral', 'royalblue', 'royalblue', 'royalblue', 'lightcoral', 'lightcoral', 'royalblue', 'royalblue', 'royalblue', 'lightcoral', 'lightcoral', 'royalblue', 'lightcoral', 'lightcoral']  
pop_list = ['M1_L23_VIP', 'M1_L5A_CS', 'M1_L5A_SST']
for layer_name in pop_list:
    ax = plt.subplot(len(pop_list), 1, i)
    ax.tick_params(axis='both',length = 5.0,width = 2.0)
    if i ==1:
        plt.title('M1_activity\n')
    if i==len(pop_list):
...
save_results_to = '/home/morteza/Desktop/reports/190925 (M1_activity)/'
#plt.savefig(save_results_to + 'M1_activity' +'.jpeg', bbox_inches='tight')
