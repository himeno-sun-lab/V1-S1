...
def get_micro_columns_data(ctx_M1_layers,target_layer_name, layer_name, micro_radius):
    ...
    for i in range(len(circle_center)):
        ...
        print( 'number of neurons in channel ', str( i ), 'for ', layer_name, ' : ', str( len( circle_gids[i] ) ) )
...
def layer_poisson_generator(layer_gid, layer_name, params={"withgid": True, "withtime": True, "to_file": False}):
    ...
    if layer_name == 'M1_L23_CC':
        PSG_arrow = nest.Create('poisson_generator', 1, params={'rate': 150.0}) #, 'label': layer_name})
...
for n in range(num_trials):
    if n == 0:
        ...
        nest.SetStatus( poisson_gens [2], {'origin': nest.GetKernelStatus() ['time'], "start": (num_trials * simulation_time) + (2 * record_intervals), 'stop': (num_trials * simulation_time) + (3 * record_intervals)} )
...
for layer_name in ctx_M1_layers.keys():
    ...
    for n in range( len( circle_col_gids [layer_name] ) ):
        freq_micro_col [layer_name][n] = spike_sort_count_freq( num_trials, spike_t_micro_circle [layer_name][n], sim_params ['initial_ignore'], sim_params ['simDuration'], circle_col_gids[layer_name][n], bin_list ) [0]
        freq_micro_col_rest [layer_name] [n] = spike_sort_count_freq( num_trials, spike_t_micro_circle [layer_name] [n], sim_params ['initial_ignore'], sim_params ['simDuration'], circle_col_gids [layer_name] [n], bin_list ) [1]
...
for n in range(len( circle_col_gids[layer_name])):
    ...
    save_results_to = '/home/morteza/Desktop/reports/191119 (quantify changes_micro_col)/L23_micro & L5B_macro/L23_CC & L23_VIP & L5B_PT/'
