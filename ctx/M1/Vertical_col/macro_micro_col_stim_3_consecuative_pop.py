...
# -*- coding: utf-8 -*-
########################################################################################################################
# In this script I design an macro col and seven micro column to see relation among the the micro columns
# one macro column in center and 7 micro around
...
def get_input_column_layers_ctx_M1(ctx_M1_layers, target_layer_name, layer_name, radius):
    ...
    for nn in range(len(layer_gids)):
        if np.linalg.norm([(neuron_positions[nn][0] - circle_center[0][0]), (neuron_positions[nn][1] - circle_center[0][1])]) <= radius:
            ...
            inside_poses.append(neuron_positions[nn])
        else:
            outside_gids.append(layer_gids[nn])

    if len(inside_poses) >= 1:
        np.savetxt( './log/' + layer_name + '_macro_column.txt', inside_poses, fmt='%.3f' )
...
################################################################
def get_micro_columns_data(ctx_M1_layers,target_layer_name, layer_name, micro_radius):
    ...
    for i in range(len(circle_center)):
        for nn in range(len(layer_gids)):
            if np.linalg.norm([(neuron_positions[nn][0] - circle_center[i][0]), (neuron_positions[nn][1] - circle_center[i][1])]) <= micro_radius:
                circle_gids[i].append(layer_gids[nn])
                circle_poses[i].append(neuron_positions[nn])

        print( 'number of neurons in micro column ', str( i ), 'for ', layer_name, ' is: ', str( len( circle_gids[i] ) ) )
        if len(circle_poses[i] ) >= 1:
            np.savetxt( './log/' + layer_name + '_micro_column ' + str(i) + '.txt', circle_poses[i], fmt='%.3f' )
...
def micro_col_spike_detector(circle_gids, layer_name, record_onset, params = {"withgid": True, "withtime": True, "to_file": False}):
    ...
    for i in range(len(circle_gids)):
        print( 'spike detector for ' + layer_name + 'in micro_col number' + str(i) )
        params.update( {'label': layer_name, "start": float(record_onset)} )
...
def average_over_trials(num_trials, var):
    ...
    for n in range(5000):
        ...
        for num in range (num_trials):
            t1 = var[n+ (num_trials*5000) + (num*5000)] + t1
...
###############################################################
def freq_of_one_time_window(spike, num_trials, simulation_time, bin_list, layer_gids):
    spike_trials = {}
    if spike.size == 0:
        for n in range(num_trials):
            spike_trials[n]= np.array([])
    ...
    for n in range( num_trials - 1 ):
        if spike_trials[n+1] == []:
            ...
            spike_trials[n+1] = spike_trials[n+1] - ((n+1)*simulation_time)

    spike_count_trials = {}
    freq_trials = {}
    for n in range(num_trials):
        spike_trials[n] = np.sort( spike_trials[n] )
        if spike_trials[n] == []:
            spike_count_trials[n] = [0]*len(bin_list-1)
        spike_count_trials[n] = np.histogram( (spike_trials[n].tolist()), bins=bin_list) [0]
...
def spike_sort_count_freq(num_trials, spike_t, initial_ignore, simulation_time, layer_gids, bin_list):
    ...
    spike_t_window_1 = np.array([])
    spike_t_window_2 = np.array([])
    spike_t_window_3 = np.array([])
    if spike_t.size == 0:
        spike_t_rest = np.array([])
        spike_t_window_1 = np.array( [] )
        spike_t_window_2 = np.array( [] )
    ...
    for t in spike_t:  # this is separating the rest and stimulated spikes
        if t <= (num_trials * simulation_time) + initial_ignore:
            kk+= 1
        if t <= ((2* num_trials) * simulation_time) + initial_ignore:
            jj+=1
        if t <= ((3*num_trials) * simulation_time) + initial_ignore:
            qq+=1
        if t > ((3*num_trials) * simulation_time) + initial_ignore:
            spike_t_rest = spike_t [0:kk]
            spike_t_window_1 = spike_t [kk:jj]
            spike_t_window_2 = spike_t [jj:qq]
            spike_t_window_3 = spike_t [qq:]
            break
    ...
    if spike_t_window_1.size != 0:
        spike_t_window_1 = np.around(spike_t_window_1 - (initial_ignore+(num_trials*simulation_time)), decimals=2)
    if spike_t_window_2.size != 0:
        spike_t_window_2 = np.around(spike_t_window_2 - (initial_ignore+((num_trials*2)*simulation_time)), decimals=2)
    if spike_t_window_3.size != 0:
        spike_t_window_3 = np.around(spike_t_window_3 - (initial_ignore+((num_trials*3)*simulation_time)), decimals=2)

    spike_t_rest = np.sort(spike_t_rest)
    spike_t_window_1= np.sort(spike_t_window_1)
    spike_t_window_2 = np.sort(spike_t_window_2)
    spike_t_window_3 = np.sort(spike_t_window_3)

    freq_rest = freq_of_one_time_window(spike_t_rest, num_trials, simulation_time, bin_list, layer_gids)
    freq_window_1 = freq_of_one_time_window( spike_t_window_1, num_trials, simulation_time, bin_list, layer_gids )
    freq_window_2 = freq_of_one_time_window( spike_t_window_2, num_trials, simulation_time, bin_list, layer_gids )
    freq_window_3 = freq_of_one_time_window( spike_t_window_3, num_trials, simulation_time, bin_list, layer_gids )


    return freq_rest, freq_window_1, freq_window_2, freq_window_3
...
target_gids_inside, target_gids_outside = get_input_column_layers_ctx_M1(ctx_M1_layers, target_layer_name['3'], target_layer_name['3'],  radius)
...
poisson_gens.append(layer_poisson_generator(target_gids_inside, target_layer_name['3']))
...
detector_micro = {}
for layer_name in ctx_M1_layers.keys():
    circle_col_gids[layer_name] = get_micro_columns_data(ctx_M1_layers, target_layer_name['1'], layer_name, micro_radius )
    ...
    detector_micro[layer_name] = micro_col_spike_detector(circle_col_gids[layer_name], layer_name, record_onset)

target_micro_col_num = 1
poisson_gens.append(layer_poisson_generator(circle_col_gids[target_layer_name['1']][target_micro_col_num], target_layer_name['1']))
poisson_gens.append(layer_poisson_generator(circle_col_gids[target_layer_name['2']][target_micro_col_num], target_layer_name['2']))
target_micro_col_num = 2
poisson_gens.append(layer_poisson_generator(circle_col_gids[target_layer_name['1']][target_micro_col_num], target_layer_name['1']))
poisson_gens.append(layer_poisson_generator(circle_col_gids[target_layer_name['2']][target_micro_col_num], target_layer_name['2']))
...
start_time=time.time()
for n in range(4* num_trials):
    if n == 0:
        print(n)
        nest.Simulate( (initial_ignore + simulation_time))
    elif n != 0 and n < num_trials:
        print( n )
        nest.Simulate(simulation_time )
    elif n >= num_trials and n < 2 * num_trials:
        print( n )
        #nest.SetStatus(poisson_gens[0], {'origin': nest.GetKernelStatus()['time'], "start": 0.0, 'stop': 3 * record_intervals - 1.0})
        nest.SetStatus( poisson_gens [0], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0})
        nest.SetStatus( poisson_gens [1], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0} )
        nest.SetStatus( poisson_gens [2], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0} )
        nest.Simulate(simulation_time)
    elif n >= 2* num_trials and n < 3* num_trials:
        print( n )
        nest.SetStatus( poisson_gens [0], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0} )
        nest.SetStatus( poisson_gens [3], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0} )
        nest.SetStatus( poisson_gens [4], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0} )
        nest.Simulate( simulation_time)
    elif n >= 3*num_trials and n < 4* num_trials -1:
        print( n )
        nest.SetStatus( poisson_gens [0], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0} )
        nest.SetStatus( poisson_gens [1], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0} )
        nest.SetStatus( poisson_gens [2], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0} )
        nest.Simulate( simulation_time )
    elif n ==  4* num_trials -1:
        print( n )
        nest.SetStatus( poisson_gens [0], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0} )
        nest.SetStatus( poisson_gens [1], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0} )
        nest.SetStatus( poisson_gens [2], {'origin': nest.GetKernelStatus() ['time'], "start": 1.0, 'stop': record_intervals-1.0} )
...
for layer_name in ctx_M1_layers.keys():
    ...
    spike_t_micro_circle [layer_name]=[]
    for n in range(len( circle_col_gids[layer_name])):
        spike_t_micro_circle [layer_name].append([])
    for n in range(len(circle_col_gids[layer_name])):
        spike_t_micro_circle [layer_name][n] = nest.GetStatus( detector_micro [layer_name][n] ) [0] ['events'] ['times']
...
times = ((events_in[0]['times'])[0:5000])
mps_in_ave = {}
mps_out_ave = {}
g_ins_in_ave = {}
g_ins_out_ave = {}
...
for layer_name in ctx_M1_layers.keys():
    ...
    freq_micro_window_1 [layer_name] = []
    freq_micro_window_2 [layer_name] = []
    freq_micro_window_3 [layer_name] = []
    freq_micro_col_rest [layer_name] = []
    for n in range(len( circle_col_gids[layer_name])):
        freq_micro_window_1[layer_name].append([])
        freq_micro_window_2 [layer_name].append( [] )
        freq_micro_window_3 [layer_name].append( [] )
        freq_micro_col_rest [layer_name].append ([])

    for n in range( len( circle_col_gids [layer_name] ) ):
        freq_micro_col_rest [layer_name] [n] = spike_sort_count_freq( num_trials, spike_t_micro_circle [layer_name] [n], sim_params ['initial_ignore'], sim_params ['simDuration'], circle_col_gids [layer_name] [n], bin_list ) [0]
        freq_micro_window_1 [layer_name] [n] = spike_sort_count_freq( num_trials, spike_t_micro_circle [layer_name] [n], sim_params ['initial_ignore'], sim_params ['simDuration'], circle_col_gids [layer_name] [n], bin_list ) [1]
        freq_micro_window_2 [layer_name] [n] = spike_sort_count_freq( num_trials, spike_t_micro_circle [layer_name] [n], sim_params ['initial_ignore'], sim_params ['simDuration'], circle_col_gids [layer_name] [n], bin_list ) [2]
        freq_micro_window_3 [layer_name] [n] = spike_sort_count_freq( num_trials, spike_t_micro_circle [layer_name] [n], sim_params ['initial_ignore'], sim_params ['simDuration'], circle_col_gids [layer_name] [n], bin_list ) [3]
...
freq_micro_col_ave = {}
freq_micro_window_1_ave = {}
freq_micro_window_2_ave = {}
freq_micro_window_3_ave = {}
freq_micro_col_sem = {}
freq_micro_col_sd = {}
...
amp_changes_micro_window_1 = {}
amp_changes_micro_window_2 = {}
amp_changes_micro_window_3 = {}
amp_changes_micro_col_abs={}
amp_changes_micro_window_1_abs={}
amp_changes_micro_window_2_abs={}
amp_changes_micro_window_3_abs={}
for n in range( len( circle_col_gids [layer_name] ) ):
    amp_changes_micro_col_abs[n] = []
    amp_changes_micro_window_1_abs [n] = []
    amp_changes_micro_window_2_abs [n] = []
    amp_changes_micro_window_3_abs [n] = []
...
for layer_name in ctx_M1_layers.keys():
    ...
    freq_micro_window_1_ave [layer_name] = []
    freq_micro_window_2_ave [layer_name] = []
    freq_micro_window_3_ave [layer_name] = []
    ...
    amp_changes_micro_window_1 [layer_name] = []
    amp_changes_micro_window_2 [layer_name] = []
    amp_changes_micro_window_3 [layer_name] = []
    for n in range( len( circle_col_gids [layer_name] ) ):
        freq_micro_window_1_ave [layer_name].append([])
        freq_micro_window_2_ave [layer_name].append( [] )
        freq_micro_window_3_ave [layer_name].append( [] )
        ...
        amp_changes_micro_window_1 [layer_name].append( [] )
        amp_changes_micro_window_2 [layer_name].append( [] )
        amp_changes_micro_window_3 [layer_name].append( [] )



    for n in range( len( circle_col_gids [layer_name] ) ):
        freq_micro_window_1_ave [layer_name][n] = calcualte_ave_sem_sd( num_trials, freq_micro_window_1 [layer_name][n] ) [0]
        freq_micro_window_2_ave [layer_name] [n] = calcualte_ave_sem_sd( num_trials, freq_micro_window_2 [layer_name] [n] ) [0]
        freq_micro_window_3_ave [layer_name] [n] = calcualte_ave_sem_sd( num_trials, freq_micro_window_3 [layer_name] [n] ) [0]
        ...
        # amp_changes_micro_window_1[layer_name][n] = np.average( freq_micro_window_1_ave [layer_name][n] [25:50] ) - np.average(freq_micro_col_rest_ave [layer_name][n] [25:50] )
        # amp_changes_micro_window_2 [layer_name] [n] = np.average( freq_micro_window_2_ave [layer_name] [n] [25:50] ) - np.average(freq_micro_col_rest_ave [layer_name] [n] [25:50] )
        # amp_changes_micro_window_3 [layer_name] [n] = np.average( freq_micro_window_3_ave [layer_name] [n] [25:50] ) - np.average(freq_micro_col_rest_ave [layer_name] [n] [25:50] )
        amp_changes_micro_window_1 [layer_name] [n] = np.average(freq_micro_window_1_ave [layer_name] [n] [0:25] ) - np.average(freq_micro_col_rest_ave [layer_name] [n] [0:25] )
        amp_changes_micro_window_2 [layer_name] [n] = np.average(freq_micro_window_2_ave [layer_name] [n] [0:25] ) - np.average(freq_micro_col_rest_ave [layer_name] [n] [0:25] )
        amp_changes_micro_window_3 [layer_name] [n] = np.average(freq_micro_window_3_ave [layer_name] [n] [0:25] ) - np.average(freq_micro_col_rest_ave [layer_name] [n] [0:25] )
        amp_changes_micro_col_abs [n].append(amp_changes_micro_col[layer_name][n])
        amp_changes_micro_window_1_abs [n].append( amp_changes_micro_window_1 [layer_name] [n] )
        amp_changes_micro_window_2_abs [n].append( amp_changes_micro_window_2 [layer_name] [n] )
        amp_changes_micro_window_3_abs [n].append( amp_changes_micro_window_3 [layer_name] [n] )


    amp_changes_in [layer_name]= np.average( freq_in_ave [layer_name] [:25] ) - np.average(freq_in_rest_ave [layer_name] [:25] )
    amp_changes_in_abs.append(amp_changes_in [layer_name])

    amp_changes_out [layer_name] = np.average( freq_out_ave [layer_name] [:25] ) - np.average(freq_out_rest_ave [layer_name] [:25] )
...
save_results_to = '/home/morteza/Desktop/reports/191112 (micro_col_test)/'
...
for n in range(len(circle_col_gids[layer_name][n])):
    i = 1
    #ax = plt.subplot( 19, 1, i )
    figure, ax = plt.subplots()
    for layer_name in ctx_M1_layers.keys():
...
for n in range(len( circle_col_gids[layer_name])):
    ...
    plt.plot(x, amp_changes_micro_window_1_abs[n], lw=2, marker = 'o', markersize=14, label= 'col_%s' %n)#(rad,))
    plt.xlabel("\nNeuron types")
    plt.ylabel("relative change in firing rate (Hz)")
    plt.legend(loc='best')
    plt.title('Response to micro stim' + target_layer_name['1']+ ', '+ target_layer_name['2'] + ' and macro stim '+ target_layer_name['3'] + ' inside col_1_window_1',fontsize = 30)
    save_results_to = '/home/morteza/Desktop/reports/191206 (3 consecuative input)/consucuative micro columns/L23_micro & L5B_macro/L23_CC & L23_VIP & L5B_PT/'
    plt.savefig( save_results_to + target_layer_name['1'] + ' and '+ target_layer_name['2'] + ' and '+ target_layer_name['3'] + ' stim inside col no ' + str(target_micro_col_num)+ 'window_1' + '.jpeg')
...
for n in range(len( circle_col_gids[layer_name])):
    ...
    plt.rc('xtick', labelsize=25)
    plt.rc('ytick', labelsize=30)
    plt.plot(x, amp_changes_micro_window_2_abs[n], lw=2, marker = 'o', markersize=14, label= 'col_%s' %n)#(rad,))
    plt.xlabel("\nNeuron types")
    plt.ylabel("relative change in firing rate (Hz)")
    plt.legend(loc='best')
    plt.title('Response to micro stim' + target_layer_name['1']+ ', '+ target_layer_name['2'] + ' and macro stim '+ target_layer_name['3'] + ' inside col_2_window_2' , fontsize = 30)
    save_results_to = '/home/morteza/Desktop/reports/191206 (3 consecuative input)/consucuative micro columns/L23_micro & L5B_macro/L23_CC & L23_VIP & L5B_PT/'
    plt.savefig( save_results_to + target_layer_name['1'] + ' and '+ target_layer_name['2'] + ' and '+ target_layer_name['3'] + ' stim inside col no ' + str(target_micro_col_num) + 'window_2' + '.jpeg')
...
for n in range(len( circle_col_gids[layer_name])):
    ...
    plt.rc('xtick', labelsize=25)
    plt.rc('ytick', labelsize=30)
    plt.plot(x, amp_changes_micro_window_3_abs[n], lw=2, marker = 'o', markersize=14, label= 'col_%s' %n)#(rad,))
    plt.xlabel("\nNeuron types")
    plt.ylabel("relative change in firing rate (Hz)")
    plt.legend(loc='best')
    plt.title('Response to micro stim' + target_layer_name['1']+ ', '+ target_layer_name['2'] + ' and macro stim '+ target_layer_name['3'] + ' inside col_3_window_3', fontsize = 30)
    save_results_to = '/home/morteza/Desktop/reports/191206 (3 consecuative input)/consucuative micro columns/L23_micro & L5B_macro/L23_CC & L23_VIP & L5B_PT/'
    plt.savefig( save_results_to + target_layer_name['1'] + ' and '+ target_layer_name['2'] + ' and '+ target_layer_name['3'] + ' stim inside col no ' + str(target_micro_col_num) + 'window_3' + '.jpeg')
