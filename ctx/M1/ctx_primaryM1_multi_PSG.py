...
#############################################################################################################
def add_poisson_generator(layer_gid, layer_name, ignore_time, params={"withgid": True, "withtime": True, "to_file": False}):
#def layer_spike_detector(layer_gid, layer_name, params={"withgid": True, "withtime": True, "to_file": True, 'fbuffer_size': 8192}):
  ...
  '''
  #########################
  # Old tune by high weight value (~50)
  #########################
  ...
  '''
  #########################
  # New tune by lower weight value (~ 5)
  #########################
  ...
  if layer_name in ['M1_L23_CC']:
    ...
    PSG = nest.Create('poisson_generator', 1, params={'rate': 1050.0, "start": float(ini_time + 20.)})  # , 'label': layer_name})           # specific PSG for L5B_PT neurons
    nest.Connect(pre=PSG, post=nest.GetNodes(layer_gid)[0], syn_spec={'weight': 4.5, 'delay': 1.5})
  ...
  if layer_name in ['M1_L23_CC']:
    ...
    PSG = nest.Create('poisson_generator', 1, params={'rate': 1050.0, "start": float(ini_time)})  # , 'label': layer_name})           # specific PSG for L5B_PT neurons
    nest.Connect(pre=PSG, post=nest.GetNodes(layer_gid)[0], syn_spec={'weight': 4.5, 'delay': 1.5})
  elif layer_name in ['M1_L5B_PT']:
    PSG = nest.Create('poisson_generator', 1, params={'rate': 1050.0, "start": float(ini_time)})
    nest.Connect(pre=PSG, post=nest.GetNodes(layer_gid)[0], syn_spec={'weight': 4.5, 'delay': 1.5})
  elif layer_name in ['M1_L6_CT']:
    PSG = nest.Create('poisson_generator', 1, params={'rate': 1200.0, "start": float(ini_time)})
    nest.Connect(pre=PSG, post=nest.GetNodes(layer_gid)[0], syn_spec={'weight': 4.0, 'delay': 1.5})
  ...
  return PSG
######################################################
def layer_spike_detector(layer_gid, layer_name, ignore_time, params={"withgid": True, "withtime": True, "to_file": False}):
#def layer_spike_detector(layer_gid, layer_name, params={"withgid": True, "withtime": True, "to_file": True, 'fbuffer_size': 8192}):
  print ('spike detector for '+layer_name)
  ...
  # add multimeter just for to record V_m and conductance (inhibitory and excitatory) of a single cell each cell population   (2 cells by random as a example)
  nest_mm = nest.Create('multimeter', params = {"interval": 0.1, "record_from": ["V_m", "g_ex", "g_in"], "withgid": True, "to_file": True,
                                                'label': layer_name, 'withtime': True, 'use_gid_in_filename': True})  # "start": float(ignore_time),
  nest.Connect(nest_mm, [nest.GetNodes(layer_gid)[0][7]])

  ####
  nest_mn = nest.Create('multimeter', params = {"interval": 0.1, "record_from": ["V_m", "g_ex", "g_in"], "withgid": True, "to_file": True,
                                                'label': layer_name, 'withtime': True, 'use_gid_in_filename': True})  # "start": float(ignore_time),
  nest.Connect(nest_mn, [nest.GetNodes(layer_gid)[0][10]])
  ####

  # voltmeter_con=nest.Create("voltmeter", params = {'withgid':True, 'withtime':True})
  voltmeter_single=nest.Create("voltmeter", params={'to_file': False, 'label': layer_name, "withgid":True,  "withtime":True})   # 'use_gid_in_filename': True,   "start": float(ignore_time),
  # nest.Connect(pre=voltmeter_con, post= nest.GetNodes(layer_gid)[0])
  nest.Connect(voltmeter_single, [nest.GetNodes(layer_gid)[0][7]])

  return detector, nest_mm, nest_mn, voltmeter_single
...
for layer_name in ctx_M1_layers.keys():
  poisson_gens [layer_name] = add_poisson_generator( ctx_M1_layers [layer_name], layer_name, sim_params ['initial_ignore'] )
  detectors[layer_name], multimeters[layer_name], multimeters1[layer_name], voltmeters[layer_name] = layer_spike_detector(ctx_M1_layers[layer_name], layer_name, sim_params['initial_ignore'])
with open('./log/logPSG/' + 'performance.txt', 'a') as file:
...
rate = {}
for layer_name in ctx_M1_layers.keys():
  rate[layer_name] = round(average_fr(detectors[layer_name], sim_params['simDuration'], count_layer(ctx_M1_layers[layer_name])),3)
  #print('Layer '+layer_name+" :      "+str(rate[layer_name])+" Hz")
  print(str( rate[layer_name] ) + " Hz" )
  with open( './log/logPSG/' + 'report.txt', 'a' ) as file:
    file.write('Layer '+layer_name+" fires at "+ str(rate[layer_name])+" Hz" + '\n' )
...
# plotting the garphs
####################################
# graph for the membrane potential #
####################################
...
for layer_name in ctx_M1_layers.keys():
  ...
  plt.show()
  save_results_to = '/home/morteza/Desktop/reports/191011 (more stability and tune for M1)/mp/'
  plt.savefig(save_results_to + str(i) +'.jpeg')
  i=i+1

########################################################################################################################
#######################################################
# graph for the excitatory and inhibitory conductance #
#######################################################
plt.rcParams["figure.figsize"] = [64,48]
i=1
for layer_name in ctx_M1_layers.keys():
  ...
  events1 = nest.GetStatus(multimeters1[layer_name])[0]["events"]
  t = events["times"]
  plt.plot(t, events["g_ex"], t, events["g_in"], t, events1["g_ex"], t, events1["g_in"])
  plt.xlabel("time (ms)")
  plt.ylabel("synaptic conductance (nS)")
  plt.title(layer_name)
  plt.legend(("g_exc1", "g_inh1", "g_exc2", "g_inh2"))
  save_results_to = '/home/morteza/Desktop/reports/191011 (more stability and tune for M1)/con/'
  plt.savefig(save_results_to + str(i) +'.jpeg')
  i=i+1
  
########################################################################################################################
#############################
# graph for the raster plot #
#############################
i=1
for layer_name in ctx_M1_layers.keys():
  ...
  plt.rc('xtick', labelsize=40) 
  plt.rc('ytick', labelsize=40)  
  save_results_to = '/home/morteza/Desktop/reports/191011 (more stability and tune for M1)/raster/'
...
i=1
for layer_name in ctx_M1_layers.keys():
    de[layer_name] = nest.GetStatus(detectors[layer_name],keys="events")[0]
...
plt.savefig(save_results_to + 'M1_activity_raster' +'.jpeg', bbox_inches='tight')

#######################################################
# graph for the excitatory and inhibitory conductance in one graph for one sample neuron#
#######################################################
plt.rcParams ["figure.figsize"] = [8, 64]
...
t = {}
for layer_name in ctx_M1_layers.keys():
  events[layer_name] = nest.GetStatus( multimeters [layer_name] ) [0] ["events"]
  t [layer_name] = events[layer_name]["times"]
...
for layer_name in ctx_M1_layers.keys():
  ...
  ax.tick_params( axis='both', length=5.0, width=2.0 )
  if i == 1:
    plt.title( 'M1_activity_synaptic_conductance\n', fontsize = 20 )
  if i == 19:
  ...
  ax.spines ['left'].set_visible( False )
  ax.spines ['right'].set_visible( False )
  plt.plot(t[layer_name], events[layer_name]["g_ex"], t[layer_name], events[layer_name]["g_in"] )
  i = i + 1

save_results_to = '/home/morteza/Desktop/reports/191031 (final M1 tuning)/'
plt.savefig( save_results_to + 'M1_activity_conductance' + '.jpeg', bbox_inches='tight' )
