...
# bulk mode
    ...
    output_ld = os.listdir('../../main/inputs/' + sys.argv[1] + '/')
    for old in output_ld:
        if os.path.isdir('../../main/inputs/' + sys.argv[1] + '/'+ old):
            if old.isdigit():
                simulation_result_dirs_for_bulk.append('../../main/inputs/' + sys.argv[1] + '/' + old )
...
if switch_bulk_mode == False:
    filepath = '../../main/inputs/' + sys.argv[1] + '/search_parameters' + sys.argv[2] + '.sli'
# In bulk mode, use #0 setting
elif switch_bulk_mode == True:
    filepath = '../../main/inputs/' + sys.argv[1] + '/search_parameters0.sli'
print(filepath)
...
filepath = '../../main/inputs/' + sys.argv[1] + '/vLSPS.sli'
...
filepath = '../../main/inputs/' + sys.argv[1] + '/M1_parameters.sli'
target_word_array = [ "/subregion_thicknesses", "/subregion_z_upper_limits" ]
target_position_array = [ 8, 8 ]
...
if switch_bulk_mode == False:
    analysis_dir = '../../main/outputs/' + sys.argv[1] + '_a/' + sys.argv[2] + '/vLSPS/'
# bulk mode
elif switch_bulk_mode == True:
    analysis_dir = '../../main/outputs/' + sys.argv[1] + '_a/all_bulk_jobs/vLSPS/'
...
if switch_bulk_mode == False:
    mm_vLSPS_files = subprocess.getstatusoutput('find ' + str(simulation_result_dir) + ' -name mm_vLSPS_*.dat | xargs ls')[1].split('\n')
# bulk mode
elif switch_bulk_mode == True:
    for srdfb in simulation_result_dirs_for_bulk:
        mm_vLSPS_files += subprocess.getstatusoutput('find ' + str(srdfb) + ' -name mm_vLSPS_*.dat | xargs ls')[1].split('\n')
...
for i_r, r in enumerate(regions):
    # loop cell types
    for i_ct, ct in enumerate(cell_types[i_r]):
        vLSPS_map_I_ex_population_mean = 0
        vLSPS_map_I_in_population_mean = 0
